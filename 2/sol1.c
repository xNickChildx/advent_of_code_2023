...
static int get_id_if_poss(const char *str, int line_len) {
	// {#red, #green, #blue}
	...
	while (str_ptr < str + line_len) {
		...
		if (color_count[red] > MAX_RED ||
		    color_count[blue] > MAX_BLUE ||
		    color_count[green] > MAX_GREEN)
			return 0;
	}
...
int main(int argc, char ** argv) {
	...
	for (; (line_len = getline(&line, &line_alloc_len, file)) > 0; sum += line_sum)
		line_sum = get_id_if_poss(line, line_len - 1);	
